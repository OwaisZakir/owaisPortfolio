...
const DepthLayers = () => {
  ...
  const layer3Opacity = useTransform(scrollY, [0, 3000], [0.15, 0.02]);
  const gridY = useTransform(scrollY, [0, 2000], [0, 250]);
  const accentY0 = useTransform(scrollY, [0, 3000], [0, 300]);
  const accentY1 = useTransform(scrollY, [0, 3000], [0, 250]);
  const accentY2 = useTransform(scrollY, [0, 3000], [0, 200]);
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-0">
      ...
      {[0, 1, 2].map((i) => {
        const accentYValues = [accentY0, accentY1, accentY2];
        return (
          <motion.div
            key={`accent-${i}`}
